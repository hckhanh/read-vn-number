...
describe('NumberReader', function() {
  ...
  it('should read number: 1.001.500.000.000', function() {
    ...
    expect(NumberReader.read(number)).to.equal('một nghìn không trăm lẻ một tỷ năm trăm triệu')
  })

  it('should read number: 1.001.500.100.000', function() {
    const number: string = '1001500100000'
    expect(NumberReader.read(number)).to.equal('một nghìn không trăm lẻ một tỷ năm trăm triệu một trăm nghìn')
  })

  it('should read number: 1.001.500.100.001', function() {
    const number: string = '1001500100001'
    expect(NumberReader.read(number)).to.equal('một nghìn không trăm lẻ một tỷ năm trăm triệu một trăm nghìn không trăm lẻ một')
  })
